...
const EducationSection = ({ education }: EducationSectionProps) => {
  return (
    <Card className="border border-border shadow-sm">
      <CardHeader>
        <CardTitle className="text-xl text-jb-primary">Education</CardTitle>
      </CardHeader>
      <CardContent>
        {education?.length ? (
          <div className="space-y-6">
            {education.map((edu, index) => (
              <div key={edu.id || index} className="border-b border-border/50 pb-5 last:border-0">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-2">
                  <div>
                    <h3 className="text-md font-semibold text-foreground">{edu.degree} in {edu.fieldOfStudy}</h3>
