...
const FeaturedCompaniesSection: React.FC<FeaturedCompaniesSectionProps> = ({
  ...
  const renderCompanies = () => {
    if (isLoading) {
      return Array(6).fill(0).map((_, index) => (
        <motion.div key={`skeleton-${index}`} variants={cardVariants} whileHover="hover">
          <Card className="border-none shadow-sm h-full">
            <CardContent className="p-4 flex flex-col items-center justify-center h-32">
              <Skeleton className="w-16 h-16 rounded-md mb-2" />
              <Skeleton className="h-4 w-24" />
    ...
    if (!companiesData || companiesData.companies.length === 0) {
      return Array(6).fill(0).map((_, index) => (
        <motion.div key={`placeholder-${index}`} variants={cardVariants} whileHover="hover">
          <Card className="border-none shadow-sm hover:bg-jb-surface h-full">
            <CardContent className="p-4 flex flex-col items-center justify-center h-32">
              <div className="w-16 h-16 bg-gradient-to-br from-jb-bg to-jb-success/30 rounded-md flex items-center justify-center mb-2 shadow-sm">
                {index % 3 === 0 && <Building className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />}
                {index % 3 === 1 && <Globe className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />}
                {index % 3 === 2 && <Users className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />}
              </div>
              <p className="text-sm font-medium text-jb-text">Company {index + 1}</p>
            </CardContent>
          </Card>
        </motion.div>
      ));
    ...
    return companiesData.companies.map((company) => {
      const industryIcons: { [key: string]: React.ReactNode } = {
        'Technology': <Globe className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />,
        'Finance': <Trophy className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />,
        'Healthcare': <Users className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />,
        'default': <Building className="w-6 h-6 md:w-8 md:h-8 text-jb-primary/70" />,
      };

      const industryIcon = industryIcons[company.industry] || industryIcons.default;

      return (
        <motion.div key={company.id} variants={cardVariants} whileHover="hover">
          <Link to={`/companies/${company.id}`} className="block h-full">
            <Card className="border-none shadow-sm hover:bg-jb-surface h-full">
              <CardContent className="p-4 flex flex-col items-center justify-center h-32">
                {company.logo ? (
                ...
                <p className="text-xs md:text-sm font-medium text-jb-text text-center line-clamp-1">{company.name}</p>
                <p className="text-xs text-jb-text-muted hidden md:block">{company.industry}</p>
              </CardContent>
            </Card>
          </Link>
        </motion.div>
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <motion.h2 className="text-2xl font-bold mb-4 inline-block relative" variants={fadeIn}>
            Featured Companies
            <span className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-24 h-1 bg-jb-primary"></span>
          </motion.h2>
