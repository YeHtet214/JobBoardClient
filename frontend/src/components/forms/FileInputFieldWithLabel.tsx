...
const FileInputFieldWithLabel: React.FC<FileInputFieldWithLabelProps> = (props) => {
  ...
  const fileValue = isFormikMode ? field.value : props.formik === false ? props.value : null;
  const hasError = isFormikMode 
    ? (meta.touched && meta.error) || fileError 
    : (props.formik === false && props.errors && props.errors[name]) || fileError;
  const errorMessage = isFormikMode 
    ? fileError || meta.error 
    : fileError || (props.formik === false && props.errors && props.errors[name]?.message);
  ...
  const handleNonFormikFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
